...
window.UpdateProductComponent = React.createClass({
  ...
  componentDidMount: function() {
    ...
    // read categories
    this.serverRequestCat = $.get("http://localhost/canhphong.github.io/ReactJS_API/api/category/read.php", function(categories) {
      this.setState({categories: categories.records});
    }.bind(this));

    // read one product data
    var productId = this.props.productId;
    this.serverRequestProd = $.get("http://localhost/canhphong.github.io/ReactJS_API/api/product/read_one.php?id=" + productId, function(product) {
  ...
  onSave: function(e) {
    ...
    $.ajax({
      url: "http://localhost/canhphong.github.io/ReactJS_API/api/product/update.php",
      type: "POST",
        ...
        <table className='table table-bordered table-hover'>
          <tbody>
            ...
            <tr>
              <td>Description</td>
              <td>
                <textarea type='text' className='form-control' required="required" value={this.state.description} onChange={this.onDescriptionChange}></textarea>
              </td>
            </tr>
            ...
            <tr>
              <td>Price ($)</td>
              <td>
                <input type='number' step="0.01" className='form-control' value={this.state.price} required="required" onChange={this.onPriceChange}/>
              </td>
            </tr>
